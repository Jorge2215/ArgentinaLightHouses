# Squad Team

> ArgentinaLightHouses

## Coordinator

| Name | Role | Notes |
|------|------|-------|
| Squad | Coordinator | Routes work, enforces handoffs and reviewer gates. |

## Members

| Name | Role | Charter | Status |
|------|------|---------|--------|
| Gandalf | Lead | .squad/agents/gandalf/charter.md | active |
| Gimli | Backend Dev | .squad/agents/gimli/charter.md | active |
| Legolas | Frontend Dev | .squad/agents/legolas/charter.md | active |
| Aragorn | Tester | .squad/agents/aragorn/charter.md | active |
| Scribe | Session Logger | .squad/agents/scribe/charter.md | active |
| Ralph | Work Monitor | .squad/agents/ralph/charter.md | active |

## Project Context

- **Project:** ArgentinaLightHouses
- **Stack:** C# / ASP.NET Core (.NET 10) Razor Pages
- **Description:** A web application showcasing Argentina's lighthouses, with lighthouse data, models, and services
- **User:** Jorge2215
- **Created:** 2026-05-03
- **Team hired:** 2026-05-06
- **Universe:** The Lord of the Rings (user-selected)
